    ...
    <>
      ...
      <div tw="relative bg-gray-50 overflow-hidden ">
        <div tw="max-w-7xl mx-auto">
          <div tw="relative z-10 pb-8 bg-gray-50 sm:pb-16 md:pb-20 lg:max-w-2xl lg:w-full lg:pb-28 xl:pb-32 ">
            <svg
              tw="hidden lg:block absolute right-0 inset-y-0 h-full w-48 text-gray-50 transform translate-x-1/2"
              fill="currentColor"
              viewBox="0 0 100 100"
            ...
            <main tw="mt-10 mx-auto max-w-7xl px-4 sm:mt-12 sm:px-6 md:mt-16 lg:mt-20 lg:px-8 xl:mt-28 ">
              <div tw="sm:text-center lg:text-left">
                <h1 tw="text-4xl tracking-tight font-extrabold text-gray-900 sm:text-5xl md:text-6xl">
                  <span tw="block xl:inline">{t.heroTitle}</span>{' '}
                  <span tw="block text-indigo-600 xl:inline">{t.heroSubtitle}</span>
                </h1>
                ...
                <div tw="mt-5 sm:mt-8 sm:flex sm:justify-center lg:justify-start">
                  <div tw="rounded-md shadow">
                    <HeroButton href="#" tw="text-white bg-indigo-600 hover:bg-indigo-700">
                      {t.heroCtaPrimary}
                    </HeroButton>
                  </div>
                  <div tw="mt-3 sm:mt-0 sm:ml-3">
                    <HeroButton href="#" tw="text-indigo-700 bg-indigo-100 hover:bg-indigo-200">
                      {t.heroCtaSecondary}
                    </HeroButton>
                  </div>
                </div>
...
function FeatureList({ t }: Props) {
  return (
    <div tw="bg-white overflow-hidden">
      <div tw="relative max-w-7xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
        <DotsPatternSVG
          width="404"
          height="784"
          tw="absolute top-0 left-full transform -translate-x-1/2 -translate-y-3/4 lg:left-auto lg:right-full lg:translate-x-2/3 lg:translate-y-1/4"
        />

        <div tw="relative lg:grid lg:grid-cols-3 lg:gap-x-8">
          <div tw="lg:col-span-1">
            <h2 tw="text-3xl font-extrabold tracking-tight text-gray-900 sm:text-4xl">{t.featuresTitle}</h2>
          </div>
          <dl tw="mt-10 space-y-10 sm:space-y-0 sm:grid sm:grid-cols-2 sm:gap-x-8 sm:gap-y-10 lg:mt-0 lg:col-span-2">
            {t.featuresCollection.items.map((item) => (
              <FeatureItem
                key={item.sys.id}
                title={item.title}
                description={item.content}
                logo={<GlobeIcon tw="h-6 w-6" />}
              />
            ))}
            {/* <ScaleIcon tw="h-6 w-6" /> */}
            {/* <LightningBoltIcon tw="h-6 w-6" /> */}
            {/* <EmailIcon tw="h-6 w-6" /> */}
          </dl>
        </div>
      </div>
    </div>
    ...
    <div tw="py-16 bg-gray-50 overflow-hidden">
      <div tw="max-w-7xl mx-auto px-4 space-y-8 sm:px-6 lg:px-8">
        <div tw="text-base max-w-prose mx-auto lg:max-w-none">
          <h2 tw="text-base text-indigo-600 font-semibold tracking-wide uppercase">{t.explainerTitle}</h2>
        ...
        <div tw="relative z-10 text-base max-w-prose mx-auto lg:max-w-5xl lg:mx-0 lg:pr-72">
          <p tw="text-lg text-gray-500">{firstParagraph.content}</p>
        </div>
        <div tw="lg:grid lg:grid-cols-2 lg:gap-8 lg:items-start">
          <div tw="relative z-10">
            ...
            <div tw="mt-10 flex text-base max-w-prose mx-auto lg:max-w-none">
              <div tw="rounded-md shadow">
                <ExplainerButton href="#" tw="text-white bg-indigo-600 hover:bg-indigo-700">
                  {t.explainerCtaPrimary}
                </ExplainerButton>
              </div>
              <div tw="rounded-md shadow ml-4">
                <ExplainerButton href="#" tw="text-indigo-600 bg-white hover:bg-gray-50">
...
function HeroButton({ children, ...props }: AnchorProps) {
  return (
    <a
      tw="w-full flex items-center justify-center px-8 py-3 border border-transparent text-base font-medium rounded-md md:py-4 md:text-lg md:px-10"
      {...props}
    >
      {children}
    </a>
...
function ExplainerButton({ children, ...props }: AnchorProps) {
  return (
    <a
      tw="w-full flex items-center justify-center px-5 py-3 border border-transparent text-base font-medium rounded-md"
      {...props}
    >
      {children}
    </a>
...
function FeatureItem({ title, description, logo }: { title: string; description: string; logo?: React.ReactChild }) {
  return (
    <div>
      <div tw="flex items-center justify-center h-12 w-12 rounded-md bg-indigo-500 text-white">{logo}</div>
      <div tw="mt-5">
        <dt tw="text-lg leading-6 font-medium text-gray-900">{title}</dt>
        <dd tw="mt-2 text-base text-gray-500">{description}</dd>
      </div>
    </div>
  );
}
  ...
  return (
    ...
    /> */}
        ...
        <div tw="relative text-lg text-gray-700 font-medium mt-8">
          ...
          <p tw="relative">{testimonial.review}</p>
        </div>
      </div>
      <cite tw="relative flex items-center sm:items-start bg-indigo-600 rounded-b-lg not-italic py-5 px-6 sm:py-5 sm:pl-12 sm:pr-10 sm:mt-10">
        ...
        <span tw="relative ml-4 text-indigo-200 font-semibold leading-6 sm:ml-24 sm:pl-1">
          <p tw="text-white font-semibold sm:inline">{testimonial.authorName}</p>{' '}
          <p tw="sm:inline">{testimonial.authorSuffix}</p>
        </span>
      </cite>
    </blockquote>
  );
